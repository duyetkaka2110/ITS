...
(function (factory) {
	...
	$.jstree.core.prototype = {
		...
		rename_node: function (obj, val) {
			...
			this.check("rename_node", obj, this.get_parent(obj), val);
			if (val in c && val !== obj.text) {
				val += "(" + (c[val] + 1) + ")";
			}
		...
		copy_node: function (obj, par, pos, callback, is_loaded, skip_redraw, origin) {
			...
			switch (pos) {
				case "before":
					pos = $.inArray(obj.id, new_par.children);
					break;
				case "after":
					pos = $.inArray(obj.id, new_par.children) + 1;
					break;
				case "inside":
					pos = $.inArray(obj.id, new_par.children) + 1;
					break;
		...
		paste: function (obj, pos) {
			pos = "inside"
			obj = this.get_node(obj);
			if (!obj) { obj = this._model.data[0] } // root folder
		...
		this.check = function (chk, obj, par, pos, more) {
			...
			switch (chk) {
				...
				case "copy_node":
					...
					return false;
				case "move_node":
					i = ((obj.parent === par.id && (!more || !more.is_multi)) || (n in c) === false);
					if (!i) {
